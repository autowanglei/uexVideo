...
public class VideoPlayerActivityForViewToWeb extends Activity implements OnPreparedListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent == null || ((VideoPlayerConfig) intent.getSerializableExtra("playerConfig"))
                == null) {// 路径不存在
    ...
    private void initViews() {
        ...
        m_ivScreenAdjust = (ImageView) findViewById(finder.getId
                ("plugin_video_player_iv_screen_adjust"));
        m_bottomLayer = (RelativeLayout) findViewById(finder.getId
                ("plugin_video_player_bottom_layout"));
    ...
    private void initMediaPlayer(String path) {
        ...
        try {
            if (path.startsWith(BUtility.F_HTTP_PATH) || path.startsWith(BUtility.F_FILE_SCHEMA)
                ...
                final AssetFileDescriptor descriptor = BUtility.getFileDescriptorByResPath(this,
                        path);
                if (descriptor == null) {
    ...
    private void toogleFullScreen() {
        ...
        if (scrollWithWeb) {
            ...
            if (lp.width == AbsoluteLayout.LayoutParams.MATCH_PARENT
                ...
                newLayout = new android.widget.AbsoluteLayout.LayoutParams(w_activity,
                        h_activity, x_activity, y_activity);
            } else { //如果之前的状态不是全屏，则恢复到全屏状态。
                newLayout = new android.widget.AbsoluteLayout.LayoutParams(LayoutParams
                        .MATCH_PARENT, LayoutParams.MATCH_PARENT, 0, 0);
            }
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        ...
        //alertMessage(finder.getString
        // ("plugin_video_can_not_support_this_format_video_playback"), true);
        onPlayerStatusChange(PALYER_STATUS_ERROR);
        return true;
    }
    ...
    private void alertMessage(String message, final boolean exitOnConfirm) {
        ...
        alertDialog = new AlertDialog.Builder(this).setTitle(finder.getString("prompt"))
                .setMessage(message)
                .setCancelable(false)
                .setPositiveButton(finder.getString("confirm"), new DialogInterface
                        .OnClickListener() {
                    @Override
    ...
    public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
        if (videoWidth == 0 || videoWidth == 0) {// 第一次进入
            if (width != 0 && height != 0) {
                videoWidth = width;
                videoHeight = height;
                BDebug.log("Screen W:" + screenWidth + "  H:" + screenHeight + "    Video W:" +
                        videoWidth + "  H:"
                        + videoHeight);
            } else {
    ...
    private void showProgressDialog() {
        ...
        Animation animation = AnimationUtils.loadAnimation(this, finder.getAnimId
                ("plugin_video_rotate_loading"));
